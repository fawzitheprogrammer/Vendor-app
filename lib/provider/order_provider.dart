...
class OrderProvider extends ChangeNotifier {
  ...
  Future<ApiResponse> assignThirdPartyDeliveryMan(BuildContext context, String name,String trackingId, int? orderId) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      showCustomSnackBar(getTranslated('third_party_delivery_type_successfully', Get.context!), Get.context!, isToaster: true, isError: false);
      assigning = false;
      getOrderList(Get.context!, 1,'all');
  ...
  Future<ApiResponse> editShippingAndBillingAddress({String? orderID, String? addressType, String? contactPersonName, String? phone,
    ...
    assigning = true;
    notifyListeners();
    ApiResponse apiResponse = await orderRepo!.orderAddressEdit(orderID: orderID,addressType: addressType, contactPersonName: contactPersonName,
    phone: phone, city: city, zip: zip, address: address, email: email, latitude: latitude, longitude: longitude);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      Navigator.of(Get.context!).pop();
      showCustomSnackBar(getTranslated('address_updated_successfully', Get.context!), Get.context!, isToaster: true, isError: false);
