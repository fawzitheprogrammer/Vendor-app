...
class AuthProvider with ChangeNotifier {
  ...
  Future<ApiResponse> login(BuildContext context, {String? emailAddress, String? password}) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      _isLoading = false;
     showCustomSnackBar(getTranslated('invalid_credential_or_account_not_verified_yet', Get.context!), Get.context!);
    }
    notifyListeners();
    return apiResponse;
  }
  ...
  Future<ApiResponse> registration(BuildContext context,RegisterModel registerModel) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse  response = await authRepo!.registration(_sellerProfileImage, _shopLogo, _shopBanner, secondaryBanner, registerModel);
