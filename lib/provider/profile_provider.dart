...
import 'package:wave_mall_vendor/data/model/response/response_model.dart';
import 'package:wave_mall_vendor/data/model/response/seller_info.dart';
import 'package:wave_mall_vendor/data/model/response/withdraw_model.dart';
...
class ProfileProvider with ChangeNotifier {
  ...
  Future<void> updateUserInfo(SellerModel updateUserModel, SellerBody seller, File? file, String token, String password) async {
    _isLoading = true;
    notifyListeners();
  ...
  Future<ApiResponse> updateBalance(String balance, BuildContext context) async {
    ...
    if (kDebugMode) {
      ...
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        ...
        getSellerInfo();
        _isLoading = false;
        showCustomSnackBar(getTranslated('withdraw_request_sent_successfully', Get.context!), Get.context!, isToaster: true, isError: false);
      }
      else {
        _isLoading = false;
      }
  ...
  Future<void> getNotificationList(int offset) async{
    ...
    if(response.response!.statusCode == 200) {
     if(offset == 1){
       notificationModel = NotificationItemModel.fromJson(response.response?.data);
     }else{
       notificationModel?.notification?.addAll(NotificationItemModel.fromJson(response.response?.data).notification!);
       notificationModel?.offset = NotificationItemModel.fromJson(response.response?.data).offset;
       notificationModel?.totalSize = NotificationItemModel.fromJson(response.response?.data).totalSize;
     }
    }else{
      ApiChecker.checkApi( response);
    }
