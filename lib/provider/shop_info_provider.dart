...
class ShopProvider extends ChangeNotifier {
  ...
  Future<void> updateShopInfo( {ShopModel? updateShopModel, File? file,String? minimumOrderAmount,
    ...
    http.StreamedResponse response = await shopRepo!.updateShop(shopModel!, file,
        ...
        Provider.of<AuthProvider>(Get.context!, listen: false).offerBanner,minimumOrderAmount: minimumOrderAmount,
        freeDeliveryOverAmount: freeDeliveryOverAmount, freeDeliveryStatus: freeDeliveryStatus
    );
    if (response.statusCode == 200) {
      if(fromOnOff){
        Provider.of<ProfileProvider>(Get.context!, listen: false).setFreeDeliveryStatus(freeDeliveryStatus!);
      }
