...
class CartProvider extends ChangeNotifier{
  ...
  void addToCart(BuildContext context, CartModel cartModel, {bool decreaseQuantity= false}) {
    ...
    if (_customerCartList[_customerIndex].cart!.any((e) => (e.product!.id == cartModel.product!.id &&  e.variant == cartModel.variant))) {
  ...
  Future<void> getCouponDiscount(BuildContext context,String couponCode, int? userId, double orderAmount) async {
    ...
    if(response.response!.statusCode == 200) {
      ...
      showCustomSnackBar('You got ${_couponCodeAmount.toString()} discount',Get.context!, isToaster: true, isError: false);
  ...
  Future<ApiResponse> placeOrder(BuildContext context, PlaceOrderBody placeOrderBody) async {
    ...
    if(response.response!.statusCode == 200){
      ...
      showCustomSnackBar(getTranslated('order_placed_successfully', Get.context!), Get.context!, isToaster: true, isError: false);
      _extraDiscountAmount = 0;
      _amount = 0;
      _collectedCashController.clear();
  ...
  Future<void> getProductFromScan(BuildContext context, String? productCode) async {
    ...
    if(response.response!.statusCode == 200) {
      ...
      Provider.of<ProductProvider>(Get.context!, listen: false).initData(_scanProduct!,1, Get.context!);
