...
import 'package:wave_mall_vendor/data/model/body/delivery_man_body.dart';
...
import 'package:wave_mall_vendor/data/model/response/delivery_man_detail_model.dart' as d;
import 'package:wave_mall_vendor/data/model/response/delivery_man_earning_model.dart';
import 'package:wave_mall_vendor/data/model/response/delivery_man_model.dart';
import 'package:wave_mall_vendor/data/model/response/delivery_man_order_history_model.dart';
import 'package:wave_mall_vendor/data/model/response/delivery_man_review_model.dart';
import 'package:wave_mall_vendor/data/model/response/delivery_man_withdraw_detail_model.dart';
import 'package:wave_mall_vendor/data/model/response/delivery_man_withdraw_model.dart';
import 'package:wave_mall_vendor/data/model/response/order_history_log_model.dart';
...
class DeliveryManProvider extends ChangeNotifier {
  ...
  Future<void> assignDeliveryMan(BuildContext context,int? orderId, int? deliveryManId) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      Provider.of<OrderProvider>(Get.context!, listen: false).getOrderDetails(orderId.toString());
      _isLoading = false;
      showCustomSnackBar(getTranslated('delivery_man_assign_successfully', Get.context!), Get.context!, isToaster: true, isError: false);
  ...
  Future<void> collectCashFromDeliveryMan(BuildContext context, int? deliveryManId, String amount) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
     ...
     showCustomSnackBar(getTranslated('amount_collected_from_deliveryman', Get.context!), Get.context!, isToaster: true);
    }else{
      _isLoading = false;
    }
    notifyListeners();
  ...
  Future<ApiResponse> addNewDeliveryMan(BuildContext context, DeliveryManBody deliveryManBody, {bool isUpdate = false}) async {
    ...
    if(response.response!.statusCode == 200) {
      ...
      _identityImages = [];
      isUpdate?
      showCustomSnackBar(getTranslated("delivery_man_updated_successfully", Get.context!), Get.context!, isError: false):
      showCustomSnackBar(getTranslated("delivery_man_added_successfully", Get.context!), Get.context!, isError: false);
    }else {
      _isLoading = false;
    }
    _isLoading = false;
