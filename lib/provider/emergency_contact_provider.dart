...
class EmergencyContactProvider extends ChangeNotifier {
  ...
  Future<void> statusOnOffEmergencyContact(BuildContext context, int? id, int status, int? index) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      showCustomSnackBar(map['message'], Get.context!, isToaster: true, isError: false);
    }else{
      Map map = apiResponse.response!.data;
  ...
  Future<ApiResponse> addNewEmergencyContact(BuildContext context, String name, String phone,int? id, {bool isUpdate = false}) async {
    ...
    if(response.response!.statusCode == 200) {
      ...
      isUpdate?
      showCustomSnackBar(getTranslated("contact_updated_successfully", Get.context!), Get.context!, isError: false):
      showCustomSnackBar(getTranslated("contact_added_successfully", Get.context!), Get.context!, isError: false);
    }else {
      _isLoading = false;
    }
    _isLoading = false;
