...
class CouponProvider with ChangeNotifier {
  ...
  Future<void> addNewCoupon(BuildContext context, Coupons coupons, bool update) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      update?
      showCustomSnackBar(getTranslated('coupon_updated_successfully', Get.context!), Get.context!, isError: false):
      showCustomSnackBar(getTranslated('coupon_added_successfully', Get.context!), Get.context!, isError: false);
    } else {
      _isAdd = false;
      ApiChecker.checkApi(apiResponse);
    }
  ...
  Future<void> updateCouponStatus(BuildContext context, int? id, int status, index) async {
    ...
    if (response.response!.statusCode == 200) {
      ...
      showCustomSnackBar(getTranslated('coupon_status_update_successfully', Get.context!), Get.context!, isError: false);
    } else {
  ...
  Future<void> deleteCoupon(BuildContext context, int? id) async {
    ...
    if (apiResponse.response!.statusCode == 200) {
      getCouponList(Get.context!, 1);
      _isLoading = false;
      showCustomSnackBar(getTranslated('coupon_deleted_successfully', Get.context!), Get.context!, isError: false);
    } else {
      _isLoading = false;
      ApiChecker.checkApi(apiResponse);
    }
