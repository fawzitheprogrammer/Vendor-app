...
import 'package:wave_mall_vendor/data/repository/product_review_repo.dart';
...
class ProductReviewProvider extends ChangeNotifier{
  ...
  Future<void> reviewStatusOnOff(BuildContext context, int? reviewId, int status, int? index) async{
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      showCustomSnackBar(getTranslated('review_status_updated_successfully', Get.context!), Get.context!, isError: false);
    }else{
      ApiChecker.checkApi(apiResponse);
    }
    notifyListeners();
